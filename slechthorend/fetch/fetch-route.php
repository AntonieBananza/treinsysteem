...
$stationFrom = $connection->prepare("SELECT id FROM `stations` WHERE stations.station = :station_from");
$stationFrom->execute([":station_from" => $stringStationFrom]);

$stationTo = $connection->prepare("SELECT id FROM `stations` WHERE stations.station = :station_to");
$stationTo->execute([":station_to" => $stringStationTo]);

$transports = $connection->prepare("SELECT transport_id FROM station_transport INNER JOIN stations ON stations.id = station_transport.station_id WHERE stations.station = :station_from OR stations.station = :station_to");
$transports->execute([":station_from" => $stringStationFrom, ":station_to" => $stringStationTo]);

$stationFromLocation = $connection->prepare("SELECT longitude_x, latitude_y FROM `stations` WHERE stations.station = :station_from");
